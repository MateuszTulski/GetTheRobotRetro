...
void ScreenBuffer::SetPixel(const Color& color, int x, int y){
	assert(mSurface);
	if(mSurface && (x > 0 && x < mSurface->w && y > 0 && y < mSurface->h)){
		SDL_LockSurface(mSurface);

		uint32_t * pixels = (uint32_t*)mSurface->pixels;
		size_t index = GetIndex(x, y);

		Color surfaceColor = Color(pixels[index]);	// Actual surface color

		pixels[index] = Color::EvaluateAlphaBlend(color, surfaceColor).GetPixelColor();

		SDL_UnlockSurface(mSurface);
	}
}
uint32_t ScreenBuffer::GetIndex(int column, int row){
	assert(mSurface);
	if(mSurface){
